...
for name, model in models.items():   # RF / LR / DT / NB / XGBoost
  ...
  /* ============================================================ 08 TRAINER (바이브코딩 MVP) */
  {
    ...
    metric: { value: "16,000+", label: "활성 사용자\n(배포 약 한 달 만에)" },
    ...
    detail: {
      ...
      data: {
        ...
        period: "2026.06.01 배포 ~ (운영 중) · 데이터 2026.07.02 기준",
        scale: "GA4 누적 활성 사용자 16,000+ · 활성 사용자당 평균 참여 1시간 00분 · Supabase 누적 로그 약 7.5만 건 (2026.6.1 배포 ~ 약 한 달, 7.2 기준)",
        preprocessing: [],
      },
      metricTabs: {
        title: "GA4 누적 추이 — 배포 후 (2026.6.1 ~ 7.2)",
        caption: "개인 프로젝트 · 광고 0원 — 6/16 카톡 입소문으로 급증(출처는 09 '측정의 함정' 설문에서 규명), 약 한 달간 꾸준히 쌓여 누적 16,000+.",
        labels: ["6/1","6/2","6/3","6/4","6/5","6/6","6/7","6/8","6/9","6/10","6/11","6/12","6/13","6/14","6/15","6/16","6/17","6/18","6/19","6/20","6/21","6/22","6/23","6/24","6/25","6/26","6/27","6/28","6/29","6/30","7/1","7/2"],
        tabs: [
          { key: "누적 사용자", value: "16,000+", unit: "명", data: [18,22,53,100,183,248,302,429,554,668,808,901,951,1011,1091,2026,3159,4083,4669,5528,6442,7138,8078,10458,12236,13115,13494,13783,14338,15225,16129,16692] },
          { key: "누적 이벤트", value: "200만+", unit: "", data: [332,557,1886,3526,7145,11321,16095,22035,33461,42713,52881,62644,66958,71735,78873,108421,187687,285179,367331,511579,657706,760718,923203,1133089,1382476,1484408,1518175,1558599,1615236,1742256,1912632,2030743] },
        ],
      },
      sections: [
        {
...
from firsts group by 1 order by 1;`,
      ...
      resultStats: [
        { value: "16,000+", label: "활성 사용자 (배포 약 한 달 만에)" },
        { value: "79%", label: "재플레이율 — 세션당 평균 13회" },
        { value: "+27점", label: "반복 연습 시 점수 향상 (89% 개선)" },
      ],
      results: [
        "AI 역량검사를 직접 준비하다 느낀 불편을 바이브코딩으로 서비스화 — 게임 4종→9종, 배포 약 한 달 만에 16,000+ 사용자(7.2 기준)",
