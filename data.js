  ...
  /* ============================================================ 08 TRAINER (바이브코딩 MVP) */
  {
    ...
    detail: {
      ...
      sections: [
        {
          ...
          body:
            "AI 역량검사(잡다 신역검) 게임을 무제한 연습하는 웹 서비스를 Claude Code 바이브코딩으로 만들었습니다.\n가위바위보·도형 회전·길 만들기·N-back 4종으로 시작해, 실사용자 피드백을 받아 약속 정하기·마법약 만들기·고양이 술래잡기·숫자 누르기·개수 비교까지 역검 게임 9종(전 게임 실전모드 포함)으로 확장했습니다.\n정확도·반응속도·점수를 기록하는 개인 통계 대시보드·목표 설정·친구 공유·피드백 창구까지 구현했습니다.\nVite+React(SPA)·Supabase·Vercel로 만들고 SEO·OG 메타까지 직접 세팅했습니다.\nAI가 코드를 작성하되, 스택 설계·SPA 라우팅·배포 디버깅·노출 지표 결정 같은 핵심 판단은 직접 내리며 진행했습니다.",
          images: [
            {
        ...
        },
        {
          ...
          body:
            "첫 사용자 소스별로 '사이트 진입 → 게임 완료' 퍼널을 분해하자(2026.06.01~12, 진입 889명) '유입량'과 '실제 완료'가 정반대였습니다. 에브리타임은 진입 349명으로 1위지만 단계마다 빠져 완료율 21.8%(완료 76명)에 그쳤습니다. 반면 링커리어는 진입이 절반 이하(166명)인데 완료율 75.9%(126명)로 에브리타임을 앞섰고, 네이버는 87.7%, bing은 79.5%였습니다. 세션 참여율도 네이버 86%·링커리어 71% vs 에브리타임 52%로 갈렸습니다. 즉 채널은 방문자 수가 아니라 '단계별 전환과 몰입'으로 평가해야 한다는 결론입니다. 다만 에브리타임은 모바일 비중이 매우 높아 측정 왜곡도 섞여 있어, 이 부분은 6번 심층 분석(측정의 함정)에서 기기까지 쪼개 분리했습니다.",
          chart: {
            type: "groupedBar",
        ...
        },
        {
          ...
          body:
            "채널 유입은 단계마다 빠지지만, 한 번이라도 플레이한 사용자는 강하게 고착됐습니다. 행동 로그 5,363건을 분석하니 404개 세션 중 79.2%가 2회 이상 재플레이했고, 세션당 평균 13.3회를 플레이했습니다. 26%는 다른 날 다시 돌아왔고, 66%는 2종 이상의 게임을 시도했습니다. 게임별로는 가장 어려운 N-back이 전체 플레이의 50%로, 사용자들이 '실전에 가까운 어려운 게임'을 가장 많이 연습한다는 신호였습니다. 단, 동질집단(코호트) 탐색에서 일간 잔존은 D+1 14%·D+2 7.7%로 낮았습니다 — 세션 내 몰입과 '다음날 다시 오기'는 다른 지표이며, 둘을 분리해 보면 제품의 고착도는 충분하고 성장의 병목은 '제품(재미)'이 아니라 '첫 플레이까지 데려오고 다시 부르는 유입'에 있다는 결론입니다.",
          chart: {
            type: "bar",
        ...
        },
        {
          ...
          body:
            "리텐션이 강한 '이유'를 라운드 단위 로그로 더 파고드니, 사용자들이 '실제로 늘어서' 다시 온다는 게 보였습니다. N-back 점수는 첫 판 평균 66점에서 3판 77점까지 도약(+11)하고 반복할수록 78점대로 안정됐으며, 같은 세션에서 3회 이상 연습한 사용자는 평균 +27점·89%가 향상됐습니다. 더 흥미로운 건 첫 점수가 낮은 사용자(50점 미만)가 오히려 평균 17.8회로 가장 많이 연습했다는 점입니다 — 낮은 점수는 이탈 신호가 아니라 동기 신호였습니다. 또 게임 내 완주율 100%로, 이탈은 게임 안이 아니라 '첫 플레이까지의 유입'에서만 일어났습니다. 즉 성장의 레버는 '마찰 제거'가 아니라 '향상을 눈에 보이게 만들고, 점수가 가장 크게 오르는 초반 2~3판까지 반복하도록 유도하는 것'이라는 결론입니다.",
          chart: {
            type: "bar",
        ...
        },
        {
          ...
          body:
            "세 분석(채널·리텐션·학습)을 실제 제품 개선으로 되돌려 배포했습니다. 학습 효과(반복 시 +20점)를 살려 결과 화면에 '지난 판 대비 +N점·개인 최고·2번째 판 넛지·연속 출석'을 붙여 재플레이를 유도했습니다. 유입이 병목이라는 진단에 따라 공유 버튼에 UTM(source=share)을 설계해, 그동안 (direct)로 묻히던 바이럴 유입을 측정 가능하게 했습니다(모바일→PC 이어가기는 원래 출처를 보존하도록 utm_term만 추가). 또한 내부 이벤트가 GA4 예약어 'source'를 써서 유입 출처를 (not set)으로 오염시키던 버그를 발견해 'ui_source'로 분리하고, 개선 전후 비교를 위해 모든 로그에 버전(v2)을 새겼습니다. 만들고 → 측정 → 분석 → 개선의 사이클을 실제로 한 바퀴 돌렸습니다.",
        },
        {
          title: "7. 업데이트 — 실사용자 피드백으로 서비스를 다시 키우다 (운영 중)",
          body:
            "배포 후 실사용자들에게 피드백이 들어오기 시작했습니다. '길 만들기 난이도가 너무 쉽다', '모든 게임에 실전모드를 넣어달라', '실제 역검처럼 게임 종류를 늘려달라' 같은 요청을 받아 곧장 업데이트로 반영했습니다. 게임을 4종에서 9종(약속 정하기·마법약 만들기·고양이 술래잡기·숫자 누르기·개수 비교 추가)으로 확장하고, 전 게임에 실전모드(정답 비공개·실제 제한시간)를 넣었습니다. 난이도는 실제 역검을 직접 해본 경험을 살려 '전체적으로 상승하되 중간에 쉬운 문제가 섞이는' 램프로 구현했습니다. 피드백을 더 쉽게 받기 위해 익명 한 줄 의견을 남기는 피드백 창구(Supabase, 60초 도배 방지)를 만들었고, 게임 종료 후에는 쌓인 플레이 데이터로 영역별 정확도를 비교하고 패턴(후반 집중력 저하·연쇄 오류 등)을 감지하는 맞춤 피드백을 제공합니다 — 표본이 적으면 단정하지 않고 침묵하도록 설계했습니다.",
          callout:
      ...
      results: [
        ...
        "행동 로그 5,363건 분석: 재플레이율 79%·세션당 평균 13회·재방문 26% → 제품 고착도는 충분, 병목은 '첫 플레이까지의 유입'",
        "라운드 단위 분석: 게임 완주율 100%(이탈은 유입에서만) · 반복 시 +27점 향상(89%) · 첫 점수 낮은 사용자가 17.8회로 최다 연습 → 개선 레버를 '마찰 제거'가 아닌 '향상 가시화·초반 반복 유도'로 재정의",
        "분석 → 제품 개선을 실제 배포(바이브코딩): 성장 피드백·2번째 판 넛지(리텐션) + 공유 UTM 바이럴 추적(유입) + 내부 이벤트의 GA4 소스 오염(not set) 수정(측정 정확도) + 버전(v2) 태깅으로 전/후 측정 설계",
        "실사용자 피드백을 받아 게임 4종 → 9종 확장·전 게임 실전모드·난이도 램프·익명 피드백 창구(Supabase)·플레이 데이터 기반 맞춤 피드백까지 배포 — 피드백 루프를 상시 운영 중",
      ],
      retro: [
        "유입량만 보면 에브리타임이 1등이지만 몰입·재방문(질)은 링커리어·네이버가 압도 — 지표는 '양'이 아니라 '목적에 맞는 질'로 봐야 함을 데이터로 체득",
        "서비스 제작(바이브코딩) → 측정(GA4·GTM·UTM) → 분석(채널·퍼널)까지 한 사이클을 직접 운영. 데이터가 더 쌓이면 코호트 재방문·게임별 이탈 단계까지 확장할 계획",
      ],
    },
  },
  {
    ...
    detail: {
      ...
      sections: [
        {
          ...
          body:
            "에브리타임은 264명(47%)으로 유입 1위지만 재방문율 7.6%·평균 참여 2분 17초로 가장 낮습니다. 반면 링커리어는 양이 절반(111명)인데 재방문율 47.7%·참여 27분 30초로 정반대입니다. 표면적으로는 '에브리타임=일회성 호기심 트래픽, 링커리어=고가치 채널'로 읽힙니다. (헤더를 누르면 정렬됩니다)",
          table: {
            sortable: true,
        ...
        },
        {
          ...
          body:
            "에브리타임은 모바일 의존도가 매우 높은 플랫폼입니다. 모바일 인앱 브라우저 환경에서는 GA4의 쿠키 기반 사용자 식별이 정상 작동하지 않을 수 있습니다. ① 모바일은 PC보다 쿠키 유지력이 낮고, ② 앱 안 인앱 브라우저로 열린 뒤 사용자가 사파리·크롬에서 다시 들어오면 다른 쿠키로 '신규'로 중복 카운트되며, ③ iOS의 ITP는 1st-party 쿠키도 7일 후 만료시켜 같은 사람이 같은 브라우저로 와도 신규로 분류됩니다(대학생층=아이폰 비율 높음 → 영향 큼). 즉 '같은 사람이 신규로 중복 집계'돼 재방문이 과소집계됐을 수 있습니다.",
          callout:
        ...
        },
        {
          ...
          body:
            "채널과 기기를 교차하자 가설의 전제부터 확인됐습니다. 에브리타임은 모바일 비율 87.9%로 압도적입니다(전체 평균 57%). 그리고 결정적으로, 같은 채널을 PC와 모바일로 쪼개니 재방문율이 극단적으로 갈렸습니다. 에브리타임은 PC 19.2% vs 모바일 5.6%로 약 3.4배, 세션 시간은 12분 12초 vs 2분 24초로 약 5배 차이입니다. 링커리어도 PC 49.4% vs 모바일 38.2%로 같은 방향입니다. 기기별 퍼널 완료율도 데스크톱 78.2% vs 모바일 24.7%로 약 3배 차이였습니다(2026.06.01~12, 진입 889명). 같은 채널·같은 콘텐츠인데 기기로 갈린다는 것은, 사용자 행동의 차이가 아니라 측정 환경의 차이라는 신호입니다. 다만 모바일의 저조에는 실제 UX 차이(키보드형 게임은 PC가 유리)도 섞여 있어, 측정 손실과 실제 행동을 분리해 해석했습니다.",
          chart: {
            type: "groupedBar",
        ...
        },
        {
          ...
          body:
            "측정 신뢰도가 높은 PC 데이터를 기준선으로 삼아 모바일을 보정했습니다. 에브리타임 PC 재방문율 19.2%를 모바일 232명에 적용하면 실제 재방문은 약 45명(GA4 보고 13명)으로, 최대 약 71%가 측정에서 누락됐을 수 있습니다. 다만 PC 사용자는 더 진지한 자기선택 집단이라 일부는 실제 행동 차이일 수 있어, 이 71%는 '측정 손실의 상한 추정'으로 봅니다. 보정 후 채널의 진짜 성격이 드러납니다: 에브리타임은 측정 손실이 큰 '도달·인지' 채널(단순 일회성 트래픽이 아님), 링커리어 PC는 신뢰도 높은 최고 가치 채널, 티스토리 PC는 재방문 61.5%로 잠재력이 미실현된 채널입니다.",
          table: {
            sortable: false,
        ...
        },
        {
          ...
          body:
            "① 측정 환경(기기·브라우저·OS)이 같은 행동도 다르게 기록합니다 — 단일 지표만으로 채널을 평가하면 오판합니다. ② 모바일 시대에 GA4의 신규/재방문 지표는 신뢰도가 제한적이라 참여율·체류시간·이벤트 발생률로 보완해야 합니다. ③ 채널마다 추적 신뢰도가 다릅니다(PC > 일반 모바일 브라우저 > 인앱 브라우저). ④ 양적 도달(에브리타임)과 질적 전환(링커리어)은 트레이드오프이자 보완 관계로, 깔때기 구조에서 둘 다 필요합니다. 액션: 링커리어에 추가 투자, 티스토리는 SEO·콘텐츠 보강(양만 늘리면 폭발적), 에브리타임은 인지 채널로 유지, 측정은 Google Signals·다중 지표 대시보드로 보완합니다.",
          callout:
      ...
      results: [
        "같은 채널을 기기로 쪼개니 재방문율이 PC 19.2% vs 모바일 5.6%(3.4배)·세션 시간 약 5배 차 → 행동이 아니라 측정 왜곡",
        "PC 기준선으로 보정 시 에브리타임 실제 재방문 약 45명(보고 13명), 최대 ~71% 측정 손실 추정(상한)",
        "재해석: 에브리타임=도달·인지 채널(단순 일회성 트래픽이 아님) / 링커리어 PC=신뢰도 높은 최고 가치 / 티스토리 PC=재방문 61.5% 잠재력",
        "측정 개선 액션 도출: Google Signals + 참여율·체류·이벤트 중심 다중 지표(신규/재방문 의존 축소)",
      ],
      retro: [
        "PC를 '깨끗한 기준선'으로 둔 보정은 측정 손실과 실제 행동 차이가 섞여 있어 '상한 추정'으로만 해석 — 단정하지 않음",
        "근본 해결은 Google Signals·서버사이드 측정·로그인 기반 식별 등 측정 인프라 보강. 표면 지표보다 '어떻게 측정됐는가'를 먼저 보는 습관을 체득",
      ],
    },
  },
];
