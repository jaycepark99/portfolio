...
const PRESETS = {
  ...
  memebox:    { label: "AI 서비스 직접 빌드·업무 자동화·지표 측정 운영", featured: "trainer", show: ["abtest", "dashboard"] },
...
for name, model in models.items():   # RF / LR / DT / NB / XGBoost
  ...
  /* ============================================================ 08 TRAINER (바이브코딩 MVP) */
  {
    ...
    metric: { value: "8,000+", label: "활성 사용자\n(배포 약 3주 만에)" },
    ...
    detail: {
      ...
      data: {
        ...
        period: "2026.06.01 배포 ~ (운영 중) · 데이터 2026.06.23 기준",
        scale: "GA4 활성 사용자 8,000+ · 활성 사용자당 평균 참여 54분 31초 · Supabase 누적 로그 약 7.5만 건 (2026.6.1 배포 ~ 약 3주, 6.23 기준)",
        preprocessing: [],
      },
      metricTabs: {
        title: "GA4 일별 추이 — 배포 후 (2026.6.1 ~ 6.23)",
        caption: "개인 프로젝트 · 광고 0원 — 6/16 카톡 입소문으로 급증(출처는 09 '측정의 함정' 설문에서 규명), 3주 만에 누적 8,000+.",
        labels: ["6/1","6/2","6/3","6/4","6/5","6/6","6/7","6/8","6/9","6/10","6/11","6/12","6/13","6/14","6/15","6/16","6/17","6/18","6/19","6/20","6/21","6/22","6/23"],
        tabs: [
          { key: "활성 사용자", value: "8,000+", unit: "명", data: [19,7,35,55,95,82,81,145,164,169,186,147,79,97,120,1008,1441,1423,1515,1779,2030,1532,2115] },
          { key: "새 사용자", value: "8,000+", unit: "명", data: [18,4,31,47,83,65,54,127,125,114,140,93,50,60,80,935,1133,924,586,859,914,696,940] },
          { key: "이벤트 수", value: "92만+", unit: "", data: [332,225,1329,1640,3619,4176,4774,5940,11426,9252,10168,9763,4314,4777,7138,29548,79266,97492,82152,144248,146127,103012,162485] },
          { key: "세션당 참여(분)", value: "최근 24분", unit: "분", data: [4.5,3.6,6.6,6.2,7.0,8.4,9.6,6.0,11.8,9.1,9.0,11.0,11.4,8.9,11.9,7.5,13.2,18.3,14.5,21.4,17.6,20.6,23.5] },
        ],
      },
      sections: [
        {
...
from firsts group by 1 order by 1;`,
      ...
      resultStats: [
        { value: "8,000+", label: "활성 사용자 (배포 약 3주 만에)" },
        { value: "79%", label: "재플레이율 — 세션당 평균 13회" },
        { value: "+27점", label: "반복 연습 시 점수 향상 (89% 개선)" },
      ],
      results: [
        "AI 역량검사를 직접 준비하다 느낀 불편을 바이브코딩으로 서비스화 — 게임 4종→9종, 배포 약 3주 만에 8,000+ 사용자(6.23 기준)",
  ...
  },
  {
    ...
    summary:
      "모바일 진입을 막던 'PC로 하세요' 배너를 6/17에 제거했습니다(블로그·지인 후기와 GA4 모바일 비율 상승에서 '사용자가 모바일 이용을 오히려 장점으로 여긴다'는 걸 확인하고 내린 결정). 그런데 하필 같은 시기 카톡 입소문으로 유입이 급등해, 모바일 전환율이 올라도 '배너 덕인지 유입 질 덕인지' 단정할 수 없었습니다. 그래서 데스크톱을 대조군으로 이중차분(DiD)했습니다 — 유입 급등은 모바일·데스크톱 공통이고, 배너는 모바일에만 있었으니까요. 모바일 진입전환은 +19%p 올랐지만 데스크톱도 +10%p 올라(공통 트렌드), 빼고 나면 배너 순효과는 약 +9%p. 지금은 모바일이 활성 사용자의 약 45%(6/23 기준)로 주력이 됐습니다. 더해 GA4가 못 보는 다크소셜(카톡)은 설문(326명)으로 삼각측량했습니다. 표면 지표를 액면 그대로 믿지 않고 대조군·설문으로 검증한 분석.",
    ...
    detail: {
      objective:
        "직접 만든 서비스에서 모바일 진입을 막던 배너를 제거하는 결정을 내렸는데, 마침 유입 급등이 겹쳐 효과를 단정할 수 없었습니다. 데스크톱을 대조군으로 삼아 '전환율 상승이 배너 덕인지 유입 질 덕인지'를 이중차분으로 분리한 분석입니다. 핵심은 '같은 날 겹친 두 변화를 어떻게 가르는가'입니다.",
      ...
      sections: [
        {
          ...
          lead: '모바일을 막던 \'PC로 하세요\' 배너를 6/17에 제거했다. 그런데 <span class="hl">같은 시기 카톡 입소문으로 유입이 폭증</span>해, 전환율이 올라도 \'배너 덕인지 유입 질 덕인지\' 알 수 없게 됐다.',
          points: [
            "왜 뺐나: 블로그·지인 후기와 GA4 모바일 비율 상승에서 '사용자가 모바일 이용을 오히려 장점으로 여긴다'는 신호를 보고, 진입을 막던 배너를 제거",
