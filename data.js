...
for name, model in models.items():   # RF / LR / DT / NB / XGBoost
  ...
  /* ============================================================ 08 TRAINER (바이브코딩 MVP) */
  {
    ...
    metric: { value: "13,000+", label: "활성 사용자\n(배포 약 4주 만에)" },
    ...
    detail: {
      ...
      data: {
        ...
        scale: "GA4 활성 사용자 13,000+ · 활성 사용자당 평균 참여 56분 58초 · Supabase 누적 로그 약 7.5만 건 (2026.6.1 배포 ~ 약 4주, 6.26 기준)",
        preprocessing: [],
      },
      metricTabs: {
        title: "GA4 일별 추이 — 배포 후 (2026.6.1 ~ 6.24)",
        caption: "개인 프로젝트 · 광고 0원 — 6/16 카톡 입소문으로 급증(출처는 09 '측정의 함정' 설문에서 규명), 약 4주 만에 누적 13,000+.",
        labels: ["6/1","6/2","6/3","6/4","6/5","6/6","6/7","6/8","6/9","6/10","6/11","6/12","6/13","6/14","6/15","6/16","6/17","6/18","6/19","6/20","6/21","6/22","6/23","6/24"],
        tabs: [
          { key: "활성 사용자", value: "13,000+", unit: "명", data: [19,7,35,55,95,82,81,145,164,169,186,147,79,97,120,1008,1441,1423,1515,1779,2030,1532,2115,3035,3070,1856] },
          { key: "새 사용자", value: "13,000+", unit: "명", data: [18,4,31,47,83,65,54,127,125,114,140,93,50,60,80,935,1133,924,586,859,914,696,940,2380,1778,879] },
          { key: "이벤트 수", value: "148만+", unit: "", data: [332,225,1329,1640,3619,4176,4774,5940,11426,9252,10168,9763,4314,4777,7138,29548,79266,97492,82152,144248,146127,103012,162485,209886,249387,101932] },
          { key: "세션당 참여(분)", value: "최근 15분", unit: "분", data: [4.5,3.6,6.6,6.2,7.0,8.4,9.6,6.0,11.8,9.1,9.0,11.0,11.4,8.9,11.9,7.5,13.2,18.3,14.5,21.4,17.6,20.6,23.5,21.1,21.8,14.7] },
        ],
      },
      sections: [
        {
...
from firsts group by 1 order by 1;`,
      ...
      resultStats: [
        { value: "13,000+", label: "활성 사용자 (배포 약 4주 만에)" },
        { value: "79%", label: "재플레이율 — 세션당 평균 13회" },
        { value: "+27점", label: "반복 연습 시 점수 향상 (89% 개선)" },
      ],
      results: [
        "AI 역량검사를 직접 준비하다 느낀 불편을 바이브코딩으로 서비스화 — 게임 4종→9종, 배포 약 4주 만에 13,000+ 사용자(6.26 기준)",
