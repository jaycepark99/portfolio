...
from firsts group by 1 order by 1;`,
  ...
  },
  {
    ...
    summary:
      "게임 완주율은 높은데 성장은 더뎠습니다. 채널을 줄세우면 유입 1위 에브리타임이 꼴찌처럼 보였지만, 퍼널을 기기로 쪼개니 진짜 병목은 '모바일 랜딩→게임 시작'(모바일 44% vs PC 79%)이었고, 그 모바일 이탈이 PC 재접속을 새 (direct)로 잡히게 해 '채널 질' 착시까지 만들고 있었습니다. 게다가 GA4가 못 보는 다크소셜(카톡 오픈채팅)은 사이트 설문(326명)으로 삼각측량해 실제 유입을 찾았습니다. 표면 지표를 액면 그대로 믿지 않고 기기·채널·측정까지 의심한 분석. → 이후 사용자 후기와 GA4 모바일 비율 상승에서 '사용자가 모바일 이용을 오히려 장점으로 여긴다'는 걸 확인하고, 진입을 막던 'PC로 하세요' 배너를 제거했습니다. 지금은 모바일이 활성 사용자의 약 45%(데스크톱 47.9%·모바일 44.6%·태블릿 7.4%, 6/23 기준)로 주력이 됐습니다.",
    ...
    detail: {
      ...
      sections: [
        {
          ...
          points: [
            ...
            "mobile_notice('PC로 하세요' 안내)가 모바일 방문의 99%(1,093/1,106)에 노출 — 이탈의 문 (→ 이후 제거, 아래 인사이트 참조)",
          ],
          chart: {
            type: "groupedBar",
        ...
        },
        {
          ...
          points: [
            "레버 1 — 모바일 첫 진입: 막는 안내를 걷어내기. 실제로 후기·GA4에서 사용자가 모바일 이용을 장점으로 여기는 걸 확인하고 'PC로 하세요' 배너를 제거함",
          ...
          callout:
            "🔑 표면 지표 → 기기로 분해 → 측정 방법론 의심 → 진짜 병목(모바일 진입) 발견. 같은 데이터에서 정반대 결론이 나왔습니다. 그리고 후기·GA4로 사용자가 모바일을 선호하는 걸 확인해 'PC로 하세요' 배너를 제거했고, 지금은 모바일이 활성 사용자의 약 45%로 주력이 됐습니다.",
        },
      ],
      resultStats: [
      ...
      results: [
        ...
        "개선 결과: 후기·GA4로 모바일 선호를 확인해 'PC로 하세요' 배너를 제거 → 모바일이 활성 사용자의 약 45%(6/23 기준)로 주력이 됨",
      ],
      retro: [
