...
const PRESETS = {
  default:    { label: "전체",            featured: "abtest",    show: ["dashboard", "rfm", "growth", "churn", "ddi", "marketing", "trainer", "aiops"] },
  ...
  nxnlabs:    { label: "AI 워크플로우·프롬프트 실험 · 제품 품질 검증 · 직접 빌드", featured: "trainer", show: ["aiops", "abtest", "growth"] },
  doers:      { label: "AI 실행 자동화·워크플로우 직접 빌드 · 운영 구조화 · 빠른 실험", featured: "trainer", show: ["aiops", "dashboard", "growth"] },
  ...
  kakaobank:  { label: "상담·문의 데이터 분류·검증 · 지표 운영·대시보드 자동화 · 데이터 정합성 · 원인 규명 A/B", featured: "aiops", show: ["dashboard", "abtest"] },
...
print(f"ARPU uplift +25.7%, p={p2:.3f}")`,
      ...
      proposal: [
        { title: "프로모션 정책 확정·타 지점 확대", desc: "2만원 이상 +10% 인센티브를 정식 정책으로 채택. 다른 지점에도 확대 시행하되, 손님층이 달라 지점 특성에 맞게 기준 구간을 다르게 설정" },
  ...
  /* ============================================================ 02 DASHBOARD */
  {
    ...
    detail: {
      objective:
        "카드를 나눠 쓰지 않아 카드·현금 내역을 매장 지출과 개인 지출로 일일이 갈라야 했고, 본사 데이터도 지점별로 따로 나와 3개 지점을 합산할 수 없었습니다. 집계에만 주 2시간이 들었고, 그동안 운영 결정은 직관에 의존했습니다. 일일 입력만으로 갱신되는 실시간 대시보드 환경을 만들어 의사결정을 데이터 기반으로 전환하는 것이 목표였습니다.",
...
WHERE amount > 0;          -- 결측/이상치 제거`,
        ...
        },
        {
          ...
          callout:
            "💡 수기 엑셀 → BI 도구 시각화 → 자동 반영 웹 도구로 진화. Looker Studio로 주 20분이던 집계가, 웹 대시보드 전환 후엔 파일을 올리고 카드·현금 지출 분류만 확인해 확정하면 돼 월 10분 수준까지 줄었습니다. '도구를 쓰는' 수준을 넘어 '도구를 만드는' 단계까지 확장했습니다.",
        },
      ],
      resultStats: [
...
for name, model in models.items():   # RF / LR / DT / NB / XGBoost
  ...
  /* ============================================================ 08 TRAINER (바이브코딩 MVP) */
  {
    ...
    summary:
      "AI 역량검사를 연습하려 했지만 공식 체험은 매번 번거로운 사전 절차를 거쳐야 해 마음 편히 반복하기 어려웠습니다. 그래서 절차 없이 바로·무제한으로 연습할 수 있는 사이트를 Claude Code 바이브코딩으로 직접 만들어 배포했습니다. GA4·GTM·UTM으로 측정 인프라를 깔고, 공개 전 지인 테스트 피드백을 받아 게임 4종 → 9종으로 키운 뒤, 만들고 → 측정 → 분석 → 개선을 며칠 단위로 도는 루프를 혼자 운영 중입니다.",
    period: "2026.05 ~ (운영 중)",
    type: "개인 프로젝트 · 바이브코딩",
    tools: ["Claude Code", "Vite·React", "Supabase", "Vercel", "GA4·GTM·UTM"],
    metric: { value: "26,000+", label: "활성 사용자\n(배포 9주)" },
    ...
    detail: {
      ...
      data: {
        ...
        period: "2026.06.01 배포 ~ (운영 중) · 데이터 2026.08.05 기준",
        scale: "GA4 누적 활성 사용자 26,000+ · 활성 사용자당 평균 참여 1시간 00분 · Supabase 누적 로그 약 7.5만 건 (2026.6.1 배포 후 7주 만에 20,000+ 달성 · 9주차인 8.5 기준 활성 누적 26,305명 · 누적 신규 26,422명 · 누적 이벤트 316만)",
        preprocessing: [],
      },
      metricTabs: {
        title: "GA4 누적 추이 — 배포 후 (2026.6.1 ~ 8.5)",
        caption: "개인 프로젝트 · 광고 0원 — 6/16 카톡 입소문으로 급증(출처는 09 '측정의 함정' 설문에서 규명), 7주 만에 20,000+ 달성 후 9주차 활성 26,000+ (아래 선은 누적 신규 사용자).",
        ...
        tabs: [
          { key: "누적 신규 사용자", value: "26,422", unit: "명", data: [18,22,53,100,183,248,302,429,554,668,808,901,951,1011,1091,2026,3159,4083,4669,5528,6442,7138,8078,10458,12236,13115,13494,13783,14338,15225,16129,16692,16947,17099,17265,17442,17583,17716,17906,18142,18338,18579,18841,18972,19167,19479,19714,19920,20123,20300,20492,20695,20982,21206,21505,22034,22463,23162,24072,24804,25232,25479,25679,25936,26162,26422] },
          { key: "누적 이벤트", value: "316만+", unit: "", data: [332,557,1886,3526,7145,11321,16095,22035,33461,42713,52881,62644,66958,71735,78873,108421,187687,285179,367331,511579,657706,760718,923203,1133089,1382476,1484408,1518175,1558599,1615236,1742256,1912632,2030743,2056848,2080780,2102944,2121249,2132548,2145617,2164696,2188946,2212981,2242965,2267539,2281779,2301510,2330313,2361125,2392885,2421864,2437105,2454207,2470778,2491472,2514169,2547666,2622509,2686818,2755830,2866384,2977368,3027728,3063108,3095218,3117971,3141888,3167797] },
        ],
      },
      sections: [
        {
          ...
          points: [
            "Claude Code로 제작 (Vite·React·Supabase·Vercel) — 게임 4종 → 공개 전 지인 테스트 피드백 받아 9종까지 확장, 전 게임 실전모드",
        ...
        },
        {
          ...
          cta: {
            to: "channel-device",
            text: "배너를 뺐는데 마침 유입도 급등 — 대조군 이중차분으로 배너 순효과(+9%p)만 가려낸 분석. 측정의 함정 심층 분석",
          },
        },
        {
          ...
          points: [
            ...
            "한 번 시작한 게임은 대부분 완주 → 병목은 게임이 아니라 '첫 진입'이었고, 배너 제거로 풀었다(→ 측정의 함정에서 이중차분으로 검증)",
          ],
          chart: {
            type: "bar",
        ...
        },
        {
          ...
          points: [
            ...
            "피드백→공개 루프: 익명 피드백 50건을 받아 수정하고 공개 '최근 개선 사항'('💬 의견 반영')에 남김 — 의견 낸 사람이 반영된 걸 다시 봄 (오답노트도 여기서 나옴)",
...
from firsts group by 1 order by 1;`,
          ...
          images: [
            {
              ...
              caption: "피드백 → 수정 → 공개 '최근 개선 사항'('💬 의견 반영')에 남김",
              kind: "capture",
            },
          ],
          callout:
        ...
        },
        {
          ...
          callout:
            "💡 재현 안 되는 버그는 말이 아니라 그 순간의 상태값이 신고와 함께 들어와야 잡힙니다 — 추측을 데이터로 바꾼, 측정으로 문제를 보이게 만든 또 하나의 사례입니다.",
        },
        {
          title: "6. '실전보다 쉽다'는 감각을 기준으로 번역 — 실전 문제지와 전수 대조 (도형 회전·길 만들기)",
          lead: '사용자는 <span class="hl">감각으로 말한다</span>("쉽다"). 그대로 난이도를 올리면 헛수정이라, 대조할 기준부터 만들었다.',
          points: [
            "문제: '실제 시험보다 쉽게 느껴진다'는 의견이 길 만들기·도형 회전에서 반복 유입 — 하지만 <b>무엇이 어떻게 쉬운지는 아무도 말해주지 않았다</b>(사용자는 내 설계를 모르니 당연)",
            "기준 만들기: 실전 문제지를 구해 <b>도형 52문항을 전수 추출</b>하고, 전·후가 같은 모양이면 채운 칸 수가 같아야 한다는 성질로 추출 자체를 검증(50/52 일치)",
            "대조 결과: 반전 비율 실전 62% vs 우리 59%, 45° 다이아 전환 실전 48% vs 우리 61%, 격자 4×4·채운 칸 7 동일 → <b>측정상 난이도는 실전과 대등</b>했고, 오히려 쉬운 1수 문제는 우리가 빼둔 상태였다",
            "진짜 원인: 난이도가 아니라 <b>연습 모드의 보조 기능</b>(라이브 프리뷰·정답 공개)이 쉽게 느끼게 만든 것 → 인위적 난이도 상향 대신 보조 기능 노출을 조정",
            "⚠️ 같은 방식으로 돌린 <b>알파벳 분석은 스스로 철회</b>했다 — 블록형 글자라 회전·반전 자동 판별이 재검증 때 값이 바뀌어 신뢰할 수 없었고, 신뢰 못 할 근거로 결론을 내지 않았다",
            "<b>길 만들기는 반대로 실제 상향이 필요했다</b>: 실전 71문제를 대조해 최소 울타리수 분포(3대 기준 2:4·3:12·4:19·5:26·6:23·7:16, 평균 5.0)를 출제 가중치로 그대로 옮김",
            "난이도의 정의를 바꿨다 — <b>정답 개수를 키우는 게 아니라</b>, 다른 색 경로가 서로 방해해 '진짜 최소'가 커지는 배치를 생성. 보이는 축(최소 울타리수)과 <b>숨은 축(해가 유일한 정도)</b>을 함께 씀",
            "⭐ 난이도를 바꾼 뒤 <b>상위% 비교 기준일을 끊었다</b> — 옛 쉬운 기록이 섞이면 사용자에게 보여주는 백분위가 불공정해지므로, 재설계 이후 기록만 비교 모수로 사용",
          ],
          callout:
            "🔑 '쉽다'는 증상이지 원인이 아닙니다 — 대조할 기준을 먼저 만들면, 고칠 곳이 난이도가 아니라 다른 데 있다는 것까지 보입니다. 측정이 불안정하면 결론을 내지 않는 것도 분석입니다.",
        },
      ],
      resultStats: [
        { value: "26,000+", label: "활성 사용자 (배포 9주)" },
        { value: "79%", label: "재플레이율 — 세션당 평균 13회" },
        { value: "+27점", label: "반복 연습 시 점수 향상 (89% 개선)" },
      ],
      results: [
        "AI 역량검사를 직접 준비하다 느낀 불편을 바이브코딩으로 서비스화 — 게임 4종→9종, 배포 7주 만에 활성 20,000+ · 9주차 26,000+ (8.5 기준 누적 신규 26,422명)",
  ...
  },
  {
    ...
    title: "측정의 함정 — 같은 날 겹친 두 변화(배너 제거·유입 급등)를 대조군으로 갈라내다",
    summary:
      "6/17, 모바일에서 'PC로 하면 더 좋다'고 권하던 배너를 없앴습니다. 그런데 하필 유입이 급등해, 전환율이 올라도 배너 덕인지 유입 덕인지 알 수 없었습니다. 데스크톱을 대조군으로 이중차분(DiD)해 순효과만 +9%p로 갈라낸 분석입니다.",
    period: "2026.06.01 ~ 06.23",
    type: "GA4 심층 분석 · jobda-trainer",
    tools: ["GA4 (탐색 분석)", "이중차분(DiD)", "이벤트·기기 퍼널", "유입 설문"],
    metric: { value: "+9%p", label: "배너 순효과 (이중차분 — 유입 급등 제거 후)" },
    chip: "프로덕트 실험 · 이중차분(DiD)",
    detail: {
      objective:
      ...
      sections: [
        {
          title: "1. 결정과 함정 — 배너를 뺐는데, 하필 유입이 급등했다",
          lead: '모바일에서 \'PC로 하면 더 좋다\'고 권하던 배너를 6/17에 제거했다. 그런데 <span class="hl">같은 시기 카톡 입소문으로 유입이 폭증</span>해, 전환율이 올라도 \'배너 덕인지 유입 질 덕인지\' 알 수 없게 됐다.',
          points: [
            "왜 뺐나: 블로그·지인 후기와 GA4 모바일 비율 상승에서 '사용자가 모바일 이용을 오히려 장점으로 여긴다'는 신호를 보고, PC를 권하던 배너를 제거",
            "함정: 배너 제거(6/17)와 카톡 입소문 급등(6/16~)이 같은 시기에 겹침 — 모바일 전환율이 올라도 '배너 효과'라 단정하면 유입 질 변화와 뒤섞인다",
            "그래서 단일 지표(모바일 전환율 상승)로 결론 내지 않고, 대조군을 세워 인과를 분리하기로 함",
          ],
        },
        {
          title: "2. 해법 — 데스크톱을 대조군으로 이중차분(DiD)",
          lead: '유입 급등은 <span class="hl">모바일·데스크톱 둘 다</span>에 왔지만, 배너는 <span class="hl">모바일에만</span> 있었다 — 그래서 데스크톱이 자연 대조군이 된다.',
          points: [
            "원리: 전환율 상승이 '유입 질' 때문이면 데스크톱도 올라야 하고, '배너' 때문이면 모바일만 올라야 한다",
            "모바일 진입전환(game_start÷page_view): 배너 전 45% → 후 64% (+19%p)",
            "데스크톱(대조군): 81% → 91% (+10%p) — 배너와 무관한데도 오른 만큼이 '유입 급등 회복 등 공통 트렌드'",
            "이중차분 = 모바일 변화(+19%p) − 데스크톱 변화(+10%p) ≈ 배너 순효과 +9%p",
          ],
          chart: {
            type: "groupedBar",
            title: "배너 제거 전·후 모바일 진입전환율 (데스크톱 = 대조군)",
            labels: ["모바일", "데스크톱(대조군)"],
            datasets: [
              { label: "배너 전 (6/1~16)", data: [45, 81] },
              { label: "배너 후 (6/17~23)", data: [64, 91] },
            ],
            note: "모바일 +19%p, 데스크톱 +10%p. 모바일이 데스크톱보다 더 오른 만큼(약 +9%p)이 배너 순효과 — 유입 급등은 양쪽 공통이라 차감되며 상쇄.",
          },
          table: {
            headers: ["", "모바일 진입전환", "데스크톱(대조군)", "차이"],
            rows: [
              ["배너 전 (6/1~16)", "45%", "81%", ""],
              ["배너 후 (6/17~23)", "64%", "91%", ""],
              ["변화", "+19%p", "+10%p", "이중차분 ≈ +9%p"],
            ],
          },
          callout:
            "🔑 raw로는 '모바일 +19%p!'지만, 데스크톱도 +10%p 올랐다(공통 트렌드). 빼고 나면 배너 순효과는 약 +9%p — 같은 날 유입이 급등해도, 대조군이 있으면 무엇이 진짜 원인인지 가를 수 있다.",
        },
        {
          ...
          points: [
            ...
            "→ 최근 급증의 정체 = 카톡 오픈채팅(대부분 모바일 유입). UTM이 없는 입소문이라 GA4엔 (direct)로 묻혀 있던 진짜 성장 동력",
          ],
          chart: {
            type: "bar",
        ...
        {
          title: "4. 인사이트 — 표면 지표를 믿지 않고 대조군·설문으로 검증",
          lead: '같은 데이터·지표라도 어떻게 측정·해석하느냐로 <span class="hl">결론이 갈린다</span>.',
          points: [
            "겹친 변화는 대조군으로 — 배너 제거와 유입 급등이 같은 날이어도, 데스크톱 대조로 배너 순효과(+9%p)만 분리했다",
            "출처는 설문으로 — GA4가 (direct)로 묻는 다크소셜(카톡·지인 73%)을 게임 후 설문으로 삼각측량했다",
            "채널을 참여도로 줄세울 땐 기기 보정이 먼저 — 모바일 많은 채널은 참여시간이 짧게 나와 '질 나쁜 채널'로 오해되기 쉽다(같은 (direct)도 PC 23분 vs 모바일 7분, 6/17~23)",
            "측정은 가볍게, 기능은 사용자 가치로만 — 재방문 측정을 위해 로그인을 강제하지 않았다(무가입·즉시연습이 강점)",
          ],
          callout:
            "🔑 전환율 상승도, 채널 줄세우기도 — 표면 지표를 액면 그대로 믿으면 틀린다. 대조군·설문·기기 보정으로 검증해야 진짜 결론이 나온다.",
        },
      ],
      resultStats: [
        { value: "+9%p", label: "배너 제거 순효과 (이중차분 — 유입 급등 제거 후)" },
        { value: "45%→64%", label: "모바일 진입전환 (배너 전→후)" },
        { value: "73%", label: "실제 유입이 카톡·지인추천 (설문 326명) — GA4엔 (direct)" },
      ],
      results: [
        "6/17 'PC로 하세요' 배너 제거 — 후기·GA4에서 사용자가 모바일을 선호함을 확인하고 내린 결정",
        "하필 카톡 입소문 급등(6/16~)과 겹쳐, 모바일 전환율 상승이 '배너 덕'인지 '유입 질 덕'인지 교란됨",
        "데스크톱을 대조군으로 이중차분: 모바일 +19%p, 데스크톱 +10%p(공통 트렌드) → 배너 순효과 ≈ +9%p",
        "결과: 모바일 진입전환 45%→64%, 모바일이 활성 사용자의 약 45%(6/23)로 주력",
        "설문 326명: 실제 유입 카톡 47%·지인 26%(다크소셜 73%) — GA4가 (direct)로 묻던 걸 삼각측량",
        "측정 교훈: 겹친 변화는 대조군으로 갈라야 하고, 채널을 참여도로 줄세울 땐 기기 보정이 먼저",
      ],
      retro: [
        "raw로 '모바일 +19%p, 배너 성공!'이라 했으면 과장 — 데스크톱 대조로 빼니 순효과는 +9%p. 같은 날 두 변화가 겹치면 대조군 없인 인과를 못 가른다는 걸 다시 확인",
  ...
  /* ============================================================ 09 AIOPS */
  {
    key: "aiops",
    domain: "automation",
    domainLabel: "AI 자동화·AX",
    title: "AI 상담봇·VOC 자동 분류 — 운영 업무를 AI로 재설계",
    summary:
      "직접 만들어 운영하는 AI 역량검사 연습 서비스에 응시 상담 질문이 들어오고, 성격이 전혀 다른 신호들이 한 창구에 섞이기 시작했습니다. 반복 질문은 Dify로 RAG 상담봇을 만들어 사이트에 붙여 시험했고, 쌓인 피드백 123건은 n8n과 Gemini로 자동 분류해 Notion 보드로 만들었습니다 — 게임에 심어둔 신고 버튼이 기기 정보와 함께 받아둔 오류 로그 52건(42%)은 버그 원인 추적으로, 사람이 쓴 의견 71건은 개선 우선순위로 갈라 썼습니다.",
    period: "2026.07 ~ (운영 중)",
    type: "개인 프로젝트 · 직접 운영 서비스에 도입",
    tools: ["Dify", "n8n", "Gemini API", "RAG", "Supabase", "Notion API"],
    ...
    detail: {
      objective:
        "직접 만든 AI 역량검사 연습 서비스의 피드백 창구에 세 가지 신호가 섞여 들어왔습니다 — 사람이 답해줘야 하는 응시 상담 질문, 기기 정보가 붙은 자동 신고 로그, 그리고 그 사이에 파묻힌 진짜 개선 의견. 반복 질문은 AI 상담봇을 만들어 사이트에 붙여 1차 응대가 가능한지 시험하고, 쌓인 피드백은 AI가 자동 분류해 보드로 정리되게 만들어 — '사람이 하던 상담·분류'를 AI로 얼마나 옮길 수 있는지 확인한 프로젝트입니다.",
      question: "고객 응대와 VOC 분류라는 반복 운영 업무를, AI로 얼마나 실제로 대체할 수 있을까?",
      data: {
        source: "직접 운영하는 AI 역량검사 연습 서비스의 피드백 데이터(Supabase) · 서비스 FAQ·게임 규칙 문서",
        period: "2026.07 도입 ~ (운영 중)",
        scale: "피드백 123건 전수 분류 · 상담봇 지식 문서(게임 9종 규칙·연습/실전 사양·응시 FAQ)",
        preprocessing: [],
      },
      sections: [
        {
          title: "1. 문제 — 피드백 창구에 섞인 세 가지 신호",
          lead: '한 창구에 <span class="hl">쓰임새가 전혀 다른 신호 세 가지</span>가 섞여 들어왔다 — 버릴 것은 하나도 없었고, 갈라야 각각 쓸 수 있었다.',
          points: [
            "피드백 창구에 응시 상담 질문이 들어오기 시작 — 예: '게임 중 혼잣말을 해도 되나요? 목소리도 제출되나요?' (사이트 개선이 아닌, 사람의 답변이 필요한 질문)",
            "게임에 심어둔 버그 신고 버튼이 기기 정보(UA)를 붙여 자동 전송 — 유용하지만 수가 많아, 수십 건을 눈으로 훑어야 진짜 의견이 보이는 상태",
            "→ 반복 질문은 <b>AI 상담봇이 1차 응대</b>, 쌓인 피드백은 <b>AI가 자동 분류</b>하는 두 갈래로 풀기로 결정",
          ],
          chart: {
            type: "doughnut",
            title: "피드백 123건 유형 분포 — 자동 신고는 수집 채널로 분리, 나머지는 AI 자동 분류",
            labels: ["자동신고 (기기 로그)", "기능요청", "실제 시험과 다름", "버그", "기타", "칭찬", "응시질문"],
            datasets: [{ label: "건수", data: [52, 26, 21, 13, 6, 3, 2] }],
            note: "오류 로그 52건(42%)은 게임에 직접 심어둔 자동 신고 버튼이 기기 정보와 함께 별도 채널로 받아둔 것 — 버리는 신호가 아니라 원인 추적의 단서다(반복된 '길 방향 전환 불가' 신고가 결국 화면 확대라는 범인으로 이어졌다). 사람이 쓴 71건은 AI가 유형·긴급도를 분류해 개선 순서로.",
          },
        },
        {
          title: "2. AI 상담봇 — FAQ를 지식으로 바꿔 RAG 봇을 사이트에 붙여보다",
          lead: '게임 규칙·응시 FAQ를 AI가 읽을 지식으로 변환하고, <span class="hl">답변 톤까지 설계</span>해 사이트에 배포했다.',
          points: [
            "지식 설계가 절반: 게임 9종 규칙·연습/실전 모드 사양·실제 응시 FAQ를 지식 문서로 재구성 — <b>내 서비스 사양은 자신 있게, 실제 시험 규정은 공식 안내 기반으로 신중하게</b> 답하도록 정보의 확신 수준을 문서 단계에서 구분",
            "Dify 챗플로우(시작 → 지식 검색[벡터 임베딩] → LLM → 답변) 구축, 사이트에 채팅 버블로 임베드 — 게임 플레이 중엔 자동으로 숨겨 UX 보호",
            "답변 품질 2사이클 튜닝: 검색이 빈손이던 임베딩 인덱스 문제를 교정하고, 면책만 하던 두루뭉술한 답을 '구체 정보 먼저, 면책은 한 문장'으로 프롬프트 재설계",
            "모델 선정도 판단: 과제 난이도(FAQ 응대) 대비 비용·응답속도를 따져 경량 모델로 시작 — 품질이 부족할 때만 올리는 방향",
          ],
          images: [
            { src: "img/aiops-bot1.png", caption: "실제 들어온 문의 그대로 — '혼잣말 해도 되나요?'에 확신 수준을 담아 답변(공식 안내 없음 → 안내문 확인 권고)", kind: "capture" },
            { src: "img/aiops-bot2.png", caption: "게임별 연습법 질문 — 연습·실전 모드 사양과 함정 규칙까지 지식 기반으로 답변", kind: "capture" },
          ],
          callout:
            "💡 실제 들어온 문의('혼잣말 해도 되나요?')가 봇 지식 확장으로 이어짐 — VOC가 지식을 키우는 루프의 시작.",
        },
        {
          title: "3. VOC 파이프라인 — n8n·Gemini로 분류를 자동화",
          lead: 'Supabase → n8n → Gemini → Notion. <span class="hl">레이트리밋·중복·엣지케이스</span>까지 설계해야 \'실제로 돌아가는\' 자동화가 된다.',
          points: [
            "n8n(셀프호스팅) 워크플로우: 피드백 조회 → Gemini가 건별로 유형·게임·긴급도·한 줄 요약 분류 → Notion 보드에 적재",
            "레이트리밋 설계: 배치 5건 + 대기 75초 + 실패 시 자동 재시도 — API 한도 안에서 안정적으로 완주하도록 흐름 제어",
            "쿼터 장애 진단: '분당 제한'으로 오판해 속도를 줄였지만 안 풀렸고, 에러를 다시 읽어 <b>하루 쿼터 소진으로 진단을 바꿔 모델 교체로 해결</b> — 쿼터는 모델별 분리라는 구조를 역이용",
            "엣지케이스: 한 피드백에 두 주제(요청+감사)가 섞이자 AI가 분류를 배열로 반환해 적재가 깨짐 → '객체 하나만, 대표 주제 하나로' 프롬프트 보강",
            "운용 모드: 이미 처리한 피드백을 기억하는 증분 처리(중복 방지)로 전환 — 실행 버튼 한 번이면 새 피드백만 분류되는 상태",
          ],
          images: [
            { src: "img/aiops-n8n.png", caption: "n8n 워크플로우 — 조회 → 중복 제거 → 배치 루프 → AI 분류 → Notion 적재 → 대기", kind: "capture" },
          ],
        },
        {
          title: "4. 결과 — 오류와 의견을 나누자 개선 우선순위가 보였다",
          lead: '분류된 보드에서 <span class="hl">무엇을 먼저 만들지</span>가 데이터로 읽힌다.',
          points: [
            "실제 의견 71건 중 1위는 버그가 아니라 <b>기능요청(26건)</b> — '고쳐줘'보다 '더 만들어줘'가 많다는 건 개발 우선순위를 VOC로 정할 수 있다는 뜻",
            "'실제 시험과 다름' 21건 — 재현 정확도 피드백이 꾸준히 들어오는 구조가 규칙 보정 같은 서비스 개선을 이끌어 왔음이 데이터로 확인",
            "게임별 집중도: 길 만들기 20 > N-back 14 > 약속 정하기 13 — 어디에 개선 리소스를 쓸지가 한눈에",
            "<b>요청 하나가 기능으로 자라는 과정</b>: '오답노트를 달라'는 요청에 6일간 4단계로 확장 — 틀린 문제 모아보기 → '맞혔지만 더 줄일 수 있던 문제'까지 → 기록 화면에서도 열람 → 시간제한 없이 다시 풀기. 로그인이 없어 브라우저 저장으로 풀었다",
            "<b>넓히기만 하지 않았다</b>: 복습을 '내 답 vs 정답' 두 보드로 키웠더니 '넘기기 불편하고 예전이 더 직관적'이라는 의견이 와 <b>한 보드로 되돌렸다</b> — 개선이라 믿은 변경도 사용자가 아니라고 하면 되돌리는 게 맞다",
            "<b>같은 기능을 두고 요구가 정면으로 갈린 사례</b>: 방향 버그 대응으로 '설치 방향 미리 고르기'를 급히 넣자, 곧바로 '그거 다시 빼달라'는 요청이 들어왔다. 신고에 자동 첨부된 환경값이 <b>coordBug=false</b>(= 버그를 안 겪는 사용자)임을 알려줘, 기본은 탭 자동 판정으로 되돌리고 방향 버튼은 <b>접어둔 채 막힌 사람만 펼치게</b> 바꿨다",
            "반복되는 응시 질문·헷갈리는 규칙이 보드에 잡히면 → 상담봇 지식·FAQ로 환류하는 운영 루프 완성",
          ],
          chart: {
            type: "bar",
            title: "실제 의견 71건 — 게임별 분포 (자동신고 제외)",
            labels: ["길 만들기", "N-back", "약속 정하기", "도형 회전하기", "가위바위보", "사이트 전반", "숫자 누르기", "마법약 만들기"],
            datasets: [{ label: "의견 수(건)", data: [20, 14, 13, 10, 6, 5, 2, 1] }],
            note: "의견이 몰리는 게임 = 사용자가 가장 진지하게 연습하는 게임이자, 개선 효과가 가장 큰 곳.",
          },
          images: [
            { src: "img/aiops-notion.png", caption: "자동 분류돼 쌓이는 Notion VOC 보드 — 유형·게임·긴급도·요약", kind: "capture" },
          ],
          callout:
            "🔑 AI가 매긴 분류를 '완료' 메시지로 믿지 않기 — 원문과 다시 대조하는 검증 단계를 파이프라인에 한 겹 넣어, 어긋난 분류가 있으면 표시되게 하고 직접 확인했다(AI가 거른 뒤 사람이 최종 판단).",
        },
      ],
      resultStats: [
        { value: "123건", label: "피드백 전수 자동 분류 (유형·게임·긴급도·요약)" },
        { value: "52건", label: "자동 신고 오류 로그 — 버그 원인 추적에 사용 (의견 71건과 분리)" },
        { value: "2사이클", label: "상담봇 답변 품질 튜닝 (인덱스 교정 · 프롬프트 재설계)" },
      ],
      results: [
        "AI 상담봇을 사이트에 붙여 시험 — 게임 규칙·연습/실전 사양·응시 환경 질문에 지식 기반으로 답하도록 구성, 답변 신뢰도·비용을 이유로 전면 확대는 보류",
        "피드백 123건 전수 자동 분류 — 이 중 52건(42%)은 게임에 심어둔 자동 신고 버튼이 별도 채널로 받아둔 오류 로그, 사람이 쓴 의견 71건(58%)만 따로 놓고 우선순위 판단",
        "실제 의견 분포: 기능요청 26 > 실제 시험과 다름 21 > 버그 13 — 개발 우선순위의 데이터 근거 확보",
        "운용 자동화: 증분 처리(중복 방지)로 전환, 새 피드백만 자동 분류되는 상태로 운영",
        "VOC 루프: 반복 질문·헷갈리는 규칙이 보드에 잡히면 봇 지식·FAQ로 환류",
      ],
      retro: [
        "쿼터 장애를 '분당 제한'으로 오판해 속도부터 줄였다 — 에러 메시지를 정확히 다시 읽고 '하루 쿼터 소진'으로 진단을 바꾸자 해결이 한 수였다. 장애 대응은 조치보다 진단이 먼저라는 걸 체감",
        "AI 자동분류를 그대로 믿지 않기 — 분류가 정확한지 원문과 다시 대조하는 검증 단계를 한 겹 넣어 직접 확인했다. AI가 거른 뒤 사람이 최종 판단하는 구조가 자동화의 신뢰를 만든다는 걸 배움",
        "상담봇은 아는 것을 자신 있게, 모르는 것을 정직하게 — '내 서비스 사양'과 '실제 시험 규정'의 확신 수준을 지식 문서 단계에서 분리한 게 답변 신뢰도의 핵심이었다",
      ],
    },
  },
];
