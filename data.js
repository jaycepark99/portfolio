...
const PRESETS = {
  ...
  hackle:     { label: "A/B·실험으로 성장 검증 · AI 서비스 직접 빌드·측정 · AI 업무 자동화(AX) · KPI·리텐션 대시보드", featured: "trainer", show: ["abtest", "aiops", "dashboard"] },
};
...
from firsts group by 1 order by 1;`,
  ...
  /* ============================================================ 09 AIOPS */
  {
    ...
    summary:
      "직접 만들어 운영하는 AI 역량검사 연습 서비스에 응시 상담 질문이 들어오고, 자동 신고 로그에 실제 사용자 의견이 파묻히기 시작했습니다. 반복 질문은 Dify로 RAG 상담봇을 만들어 1차 응대를 맡기고, 쌓인 피드백 123건은 n8n과 Gemini로 자동 분류해 Notion 보드로 만들었습니다 — 노이즈 42%를 걷어내자 개선 우선순위가 데이터로 드러났습니다.",
    period: "2026.07 ~ (운영 중)",
    ...
    detail: {
      ...
      sections: [
        {
          ...
          lead: '자동 신고 로그가 42%를 차지해 <span class="hl">진짜 의견이 노이즈에 파묻혔고</span>, 봇이 답할 수 있는 상담 질문까지 섞여 들어왔다.',
          points: [
          ...
          chart: {
            ...
            note: "자동신고(노이즈) 42%를 걷어내면 실제 의견 71건(58%)이 남는다 — 이 분리가 자동화의 첫 성과.",
          },
        },
        {
        ...
        {
          title: "4. 결과 — 노이즈를 걷어내자 개선 우선순위가 보였다",
          ...
          callout:
            "🔑 파이프라인 검증은 '성공' 메시지가 아니라 목적지 데이터의 전수 카운트로 — 중간에 유실된 78건을 카운트 대조로 잡아내 마저 채웠다.",
        },
      ],
      resultStats: [
        { value: "123건", label: "피드백 전수 자동 분류 (유형·게임·긴급도·요약)" },
        { value: "42%", label: "자동 신고 로그(노이즈) 비중 — 분리 후 실제 의견 71건 확보" },
        { value: "2사이클", label: "상담봇 답변 품질 튜닝 (인덱스 교정 · 프롬프트 재설계)" },
      ],
      results: [
        "AI 상담봇 라이브 배포 — 게임 규칙·연습/실전 사양·실제 응시 환경 질문에 지식 기반 1차 응대",
        "피드백 123건 전수 자동 분류: 자동신고 52(42%) vs 실제 의견 71(58%) — 노이즈와 시그널 분리",
      ...
      retro: [
        ...
        "파이프라인이 '성공했다'고 말해도 믿지 않기 — 목적지(Notion) 전수 카운트로 대조하니 중간 중단으로 78건이 유실돼 있었다. 데이터 파이프라인 검증의 기본을 몸으로 배움",
