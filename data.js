...
for name, model in models.items():   # RF / LR / DT / NB / XGBoost
  ...
  /* ============================================================ 08 TRAINER (바이브코딩 MVP) */
  {
    ...
    summary:
      "AI 역량검사를 직접 준비하다 '횟수 제한 없이 반복 연습하고 기록을 추적할 곳이 없다'는 불편을 느껴, Claude Code 바이브코딩으로 연습 사이트를 직접 만들어 배포했습니다. GA4·GTM·UTM으로 측정 인프라를 깔고, 실사용자 피드백을 받아 게임 4종 → 9종으로 키우며, 만들고 → 측정 → 분석 → 개선을 며칠 단위로 도는 루프를 혼자 운영 중입니다.",
    period: "2026.05 ~ (운영 중)",
    type: "개인 프로젝트 · 바이브코딩",
    ...
    detail: {
      objective:
        "AI 역량검사를 직접 준비하다 '횟수 제한 없이 반복 연습하고 기록을 추적할 곳이 없다'는 불편을 느껴, 직접 서비스를 만들어 풀었습니다. 만드는 데 그치지 않고 — 측정 인프라를 깔고(GA4·UTM), 실사용자 피드백으로 검증해, 며칠 단위로 다시 배포하는 '만들고 → 측정 → 분석 → 개선' 루프를 혼자 돌리는 프로젝트입니다.",
      question: "내가 겪은 불편을 직접 서비스로 만들어, 데이터로 검증하며 계속 키울 수 있을까?",
      data: {
        source: "직접 제작·운영하는 웹 서비스 (jobda-trainer.vercel.app)",
        period: "2026.05 ~ (운영 중) · 데이터 2026.06 기준 (운영하며 계속 갱신)",
        scale: "GA4 활성 사용자 800+ · 활성 사용자당 평균 참여 16분 27초 · UTM 멀티채널 (링커리어·에브리타임·네이버·인스타·티스토리) — 유입 계속 증가 중",
        preprocessing: [],
      },
      sections: [
        {
        ...
        {
          title: "3. 고착도 — 유입은 새도, 들어오면 끈끈하다 (리텐션)",
        ...
        },
        {
          ...
          points: [
            ...
            "한 번 시작한 게임은 대부분 끝까지 플레이 → 이탈은 게임 난이도가 아니라 '첫 플레이까지 데려오기'에서",
        ...
        {
          title: "6. 업데이트 — 실사용자 피드백으로 서비스를 다시 키우다 (운영 중)",
      ...
      results: [
        "AI 역량검사를 직접 준비하다 느낀 불편을 바이브코딩으로 서비스화 — 게임 4종→9종, 800+ 사용자 운영 중",
  ...
  },
  {
    ...
    summary:
      "게임 완주율은 높은데 성장은 더뎠습니다. 채널을 줄세우면 유입 1위 에브리타임이 꼴찌처럼 보였지만, 퍼널을 기기로 쪼개니 진짜 병목은 '모바일 랜딩→게임 시작'(모바일 44% vs PC 79%)이었고, 그 모바일 이탈이 PC 재접속을 새 (direct)로 잡히게 해 '채널 질' 착시까지 만들고 있었습니다. 표면 지표를 액면 그대로 믿지 않고 기기·측정까지 의심해 진짜 레버를 찾은 분석.",
    ...
    detail: {
      ...
      sections: [
        {
          title: "1. 관찰 — '완주율은 높은데 왜 안 크지?'",
          lead: '게임 완주율은 높은데 성장은 더디다 — 채널을 줄세우면 유입 1위 에타가 <span class="hl">완료·재방문 꼴찌</span>처럼 보인다.',
          points: [
            "게임을 시작한 세션의 완주율은 높음 → '게임 자체'는 문제없어 보였다",
            "그런데 첫 소스로 줄세우니 유입 1위 에브리타임이 완료·재방문 최저 → 표면상 '질 낮은 트래픽'",
            "여기서 멈추면 결론은 '에타 끊고 링커리어 늘려라' — 그런데 숫자를 기기로 쪼개니 달랐다",
          ],
          callout:
            "💡 표면 해석: '에브리타임은 양만 많고 질 낮다.' — 그러나 이 결론엔 측정·퍼널상의 함정이 숨어 있었습니다.",
        },
        {
          title: "2. 퍼널을 기기로 쪼개다 — 진짜 병목은 '모바일 첫 진입'",
        ...
        {
          title: "3. 그 모바일 이탈이 '채널 질' 착시까지 만들었다",
        ...
        {
          title: "4. 리텐션 장치는 만들었는데, 도달이 안 된다",
          lead: '재방문을 부르려 만든 기록 대시보드·목표 설정이 <span class="hl">정작 도달률이 낮다</span>.',
          points: [
            "기록 대시보드 페이지: 177명 방문·체류 16초 — 게임 완료자(836명) 대비 거의 안 봄",
            "목표 설정 154명 → 달성 100명(설정자의 65%) — 기능은 잘 작동하나 '설정'까지 오는 건 완료자의 18%뿐",
            "주간 재방문 W1 ~11%지만 이건 '과제'가 아님 — ① 시험 일정 잡혔을 때 바짝 쓰는 에피소드형이라 주간 리텐션 잣대가 안 맞고 ② 로그인 없어 장기 재방문은 쿠키 만료로 과소집계(재방문율로 성패 보면 오판)",
            "게임별 완료율: 도형회전 82%·가위바위보 79%는 높고, 마법약 58%로 최저(규칙 난해) → 신규 게임 개선 타깃",
          ],
          chart: {
            type: "bar",
            title: "게임별 시작→완료율 — 마법약(58%)이 최저",
            labels: ["도형 회전", "가위바위보", "N-back", "길 만들기", "마법약"],
            datasets: [{ label: "완료율(%)", data: [82, 79, 70, 69, 58] }],
            note: "도형회전·가위바위보는 80% 안팎, 신규 게임 마법약은 58%로 최저 — 규칙 난해, 개선 타깃.",
          },
          table: {
            sortable: true,
            headers: ["게임", "시작", "완료", "완료율"],
            rows: [
              ["도형 회전", "606", "496", "82%"],
              ["가위바위보", "420", "332", "79%"],
              ["N-back", "637", "446", "70%"],
          ...
          callout:
            "💡 고착도(재플레이 79%)는 충분한데, 재방문을 부르는 장치(목표·기록)의 '도달'이 또 다른 병목이었습니다.",
        },
        {
          title: "5. 인사이트 — '재미'가 아니라 '모바일 첫 진입'을 푼다",
          lead: '데이터를 액면 그대로 믿지 않고 기기·측정까지 의심하니 <span class="hl">레버가 바뀌었다</span>.',
          points: [
            "레버 1 — 모바일 첫 진입: 안내를 '막는 문'이 아니라 '모바일에서 되는 게임부터 권하는' 방향으로",
            "레버 2 — 채널 평가를 '양' 아닌 '기기 보정된 질'로: 에타=도달·인지, 링커리어·네이버 PC=고가치",
            "레버 3 — 리텐션 장치(목표·기록) 도달률을 결과 화면 동선에서 끌어올리기",
            "레버 4 — 에피소드형이라 '습관 리텐션'보다 '필요할 때 다시 찾게 하는 검색 발견성(재유입)'이 진짜 성장 레버",
            "측정: 재방문율 단일 지표 대신 기기 분리 + 참여·이벤트 다중 지표. 로그인은 측정 목적으론 안 넣음 — 가입 마찰 > 이득",
          ],
          callout:
      ...
      resultStats: [
        ...
        { value: "99%", label: "모바일 방문의 mobile_notice 노출" },
      ...
      results: [
        ...
        "기록 대시보드 177명·16초·goal_set 18%만 — 리텐션 장치 도달 실패가 별도 병목",
